...
export async function POST(request: NextRequest) {
  try {
    const { repoUrl, targetFolderName, baseDir } = await request.json();
    ...
    // Determine base directory for clone target
    let basePath: string;
    if (typeof baseDir === "string" && baseDir.trim()) {
      // Caller provided an explicit base directory — ensure it is within WORKSPACE_ROOTS
      const normalizedBase = baseDir.replace(/\\/g, "/");
      const inRoot = roots.some((root) => {
        const nr = root.replace(/\\/g, "/");
        return (
          normalizedBase === nr ||
          normalizedBase.startsWith(nr.endsWith("/") ? nr : nr + "/")
        );
      });
      if (!inRoot) {
        return NextResponse.json(
      ...
      // Fallback: use the first workspace root as the clone target
      basePath = roots[0];
    }
