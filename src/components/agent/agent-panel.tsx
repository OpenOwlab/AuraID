...
import { getOverflowThresholdChars, getMessageTextLength, getContextWindowChars, PROVIDERS, DEFAULT_PROVIDER, DEFAULT_MODEL, DEFAULT_CONTEXT_MODE, modelSupportsImages } from "@/lib/ai/models";
...
export function AgentPanel({
  ...
  const MODEL_SELECTION_STORAGE_KEY = "loopclaw-agent-model-selection";
  ...
  const handleDragStart = useCallback((e: React.MouseEvent) => {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      const newH = Math.max(88, Math.min(maxH, dragRef.current.startH + delta));
      setInputHeight(newH);
    };
    const onUp = () => {
  ...
  const [showWorkspaceImageDialog, setShowWorkspaceImageDialog] = useState(false);
  ...
  const canAttachImages = useMemo(() => {
    const provider =
      selectedProvider || (settings?.llmProvider as string | undefined) || DEFAULT_PROVIDER;
    const model =
      selectedModel || (settings?.llmModel as string | undefined) || DEFAULT_MODEL;
    return modelSupportsImages(provider, model);
  }, [selectedProvider, selectedModel, settings?.llmProvider, settings?.llmModel]);
  ...
  return (
    <div ref={containerRef} className="relative flex h-full min-w-0 flex-col bg-agent-bg text-agent-foreground font-mono text-sm overflow-hidden">
            ...
            messages[messages.length - 1].role === "user" && (
              <div className="relative my-4">
                <div className="relative flex items-center gap-3 rounded-lg border border-primary/30 bg-gradient-to-r from-primary/10 via-accent/5 to-primary/10 p-4 backdrop-blur-sm animate-glow-pulse overflow-hidden">
                  ...
                  <ThinkingIndicator label="AuraID thinking" />
                </div>
              </div>
            )}
      ...
      {isSummarizing && (
        <div className="flex items-center gap-2 px-3 py-1.5 border-t border-agent-border bg-agent-bg text-xs text-agent-accent">
          <Loader2 className="h-3 w-3 animate-spin" />
          <span>{t("memorySaving")}</span>
        </div>
      )}
      {summaryError && (
        <div className="flex items-center gap-2 px-3 py-1.5 border-t border-agent-border bg-agent-bg text-xs text-agent-error">
          <AlertCircle className="h-3 w-3" />
          <span>{t("memoryError")}: {summaryError}</span>
          <button
            onClick={() => setSummaryError(null)}
            className="ml-auto text-agent-muted hover:text-agent-foreground text-xs"
          >
            {t("dismiss")}
          </button>
        </div>
      ...
      {attachedImages.length > 0 && (
        <div className="relative z-10 px-3 py-2 border-b border-agent-border/60 bg-agent-bg/90 backdrop-blur-sm">
          <div className="flex w-full gap-2 overflow-x-auto pb-1">
            {attachedImages.map((img, index) => (
              <div
                key={`${img.path}-${img.filename}-${index}`}
                className="relative flex-shrink-0 w-[132px] rounded-md border border-agent-border bg-agent-card p-1"
              >
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="absolute right-1 top-1 z-10 flex h-4 w-4 items-center justify-center rounded-full bg-black/60 text-[9px] text-white hover:bg-black/80"
                  aria-label="Remove image"
                >
                  ×
                </button>
                <div className="flex h-20 items-center justify-center bg-black/5 dark:bg-white/5 rounded">
                  <img
                    src={img.url}
                    alt={img.filename}
                ...
                <div className="mt-1 text-[10px] leading-snug text-agent-muted truncate" title={img.path}>
                  {img.path}
                </div>
              </div>
            ))}
      ...
      <div className="relative z-10 bg-agent-bg/80 backdrop-blur-sm shrink-0 overflow-hidden" style={{ height: inputHeight }}>
        ...
        <div className="flex items-start gap-2 px-3 py-2 h-full">
          ...
          <span className="text-agent-purple font-bold shrink-0 select-none mt-2">
            &gt;
          </span>
          <textarea
            ref={inputRef}
            ...
            className="flex-1 bg-transparent text-agent-foreground placeholder:text-agent-muted outline-none text-sm font-mono resize-none h-full leading-6 py-2"
            rows={3}
            autoFocus
          />
          <div className="flex items-center gap-1 shrink-0 mt-1">
            {attachedImages.length > 0 && (
              <span className="text-[10px] font-mono tabular-nums px-1 py-0.5 rounded bg-agent-card text-agent-foreground">
            ...
            {messages.length > 0 && (
              <div
                className={`flex items-center gap-1 rounded px-1.5 py-0.5 text-[10px] font-mono tabular-nums select-none transition-colors ${
                  contextPercent >= 80
                    ? "text-[#f7768e]"
                    : contextPercent >= 50
                    ? "text-[#e0af68]"
            ...
            {isLoading && (
              <button
                onClick={handleStop}
                title={t("stop")}
                className="shrink-0 p-1.5 rounded hover:bg-agent-card-hover text-agent-error transition-colors"
              >
            ...
            {!isLoading && !isSummarizing && messages.length > 0 && (
              <button
                ...
                className="shrink-0 p-1.5 rounded hover:bg-agent-card-hover text-agent-muted hover:text-agent-foreground transition-colors"
              >
            ...
            <button
              type="button"
              onClick={() => {
                if (!aiEnabled || isSummarizing || !canAttachImages) return;
                setShowWorkspaceImageDialog(true);
              }}
              title={t("attachImageFromWorkspace")}
              className={`shrink-0 p-1.5 rounded hover:bg-agent-card-hover transition-colors ${
                canAttachImages
                  ? "text-agent-muted hover:text-agent-foreground"
                  : "text-agent-muted opacity-50 cursor-not-allowed"
              }`}
              disabled={!canAttachImages || !aiEnabled || isSummarizing}
            >
            ...
            <button
              ...
              title={canAttachImages ? t("attachImage") : t("attachImageDisabled")}
              className={`shrink-0 p-1.5 rounded hover:bg-agent-card-hover transition-colors ${
                canAttachImages
                  ? "text-agent-muted hover:text-agent-foreground"
                  : "text-agent-muted opacity-50 cursor-not-allowed"
          ...
          <input
            ...
            onChange={async (e) => {
              ...
              const uploads: AttachedImage[] = [];
              for (const file of Array.from(files)) {
                try {
                  // Read file as data URL (base64) for OpenAI-compatible /v1 APIs
                  ...
                  // 图片统一保存到「工作目录/chat-images」下，便于在文件浏览器中直接查看和管理
                  const targetDir = `${folderPath.replace(/\\+/g, "/")}/chat-images`;
              ...
              if (uploads.length > 0) {
                setAttachedImages((prev) => [...prev, ...uploads]);
              }
      ...
      {/* Workspace image picker dialog */}
      <Dialog open={showWorkspaceImageDialog} onOpenChange={(open) => {
        setShowWorkspaceImageDialog(open);
        if (!open) {
        ...
        <DialogContent className="max-w-4xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[2fr,3fr] gap-4">
            <div className="space-y-2">
              <Label className="text-xs text-agent-muted">{t("attachImageWorkspacePathPlaceholder")}</Label>
              <Input
                value={workspaceImagePath}
                onChange={(e) => setWorkspaceImagePath(e.target.value)}
                placeholder={t("attachImageWorkspacePathPlaceholder")}
                className="text-xs font-mono"
              />
              <p className="text-[11px] text-agent-muted leading-snug">
                {folderPath.replace(/\\+/g, "/")}/...
              </p>
            </div>
            <div className="h-72 rounded-lg border border-border/60 bg-background/80 overflow-hidden">
              <FileBrowser
                ...
                onFileSelect={(p) => {
                  if (!p) return;
                  setWorkspaceImagePath(p);
                }}
                selectedFilePath={workspaceImagePath || null}
              />
            </div>
          </div>
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => {
                setShowWorkspaceImageDialog(false);
                setWorkspaceImagePath("");
              }}
            >
              {tCommon("cancel")}
            ...
            <Button
              disabled={!workspaceImagePath}
              onClick={async () => {
                if (!workspaceImagePath) return;
                try {
                  const baseFolder = folderPath.replace(/\\+/g, "/");
                  const rawPath = workspaceImagePath.startsWith("/")
                    ? workspaceImagePath
                    : `${baseFolder}/${workspaceImagePath}`;
                  const res = await fetch("/api/files/chat-image-from-workspace", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                  ...
                  if (!res.ok) {
                    // eslint-disable-next-line no-console
                    console.error("Failed to copy workspace image", await res.text());
                    return;
                  }
                  const data = (await res.json()) as { path: string };
                  const rawUrl = `/api/files/raw?path=${encodeURIComponent(data.path)}`;
                  const resp = await fetch(rawUrl);
                  if (!resp.ok) {
                    // eslint-disable-next-line no-console
                    console.error("Failed to load copied image", await resp.text());
                    return;
                  }
                  ...
                  setAttachedImages((prev) => [
                    ...
                    {
                      url: dataUrl,
                      filename: data.path.split("/").slice(-1)[0],
                  ...
                  // eslint-disable-next-line no-console
                  console.error("Error attaching workspace image", err);
                }
              }}
            >
              {tCommon("confirm")}
      ...
      {/* Reduced overlay effects to keep focus on content */}
    </div>
  );
}
