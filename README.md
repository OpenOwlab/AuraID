...
一个可自托管的 AI 研究助手。将服务器文件夹作为工作空间，基于 RAG 与 AI 对话，支持丰富的技能调用和 Agent 工作流。

A self-hostable AI research assistant. Turn server-side folders into workspaces, chat with AI grounded in your documents via RAG, and leverage powerful skills and Agent workflows.
...
- 🔬 **丰富的技能调用** — 可从 Clawhub 直接导入技能或者让 agent 自动生成技能。
...
```bash
git clone https://github.com/OpenOwlab/AuraID.git
cd AuraID
npm install
```
...
## Docker 运行建议 / Docker Runtime Recommendation

考虑到本项目涉及本地文件映射、命令执行与外部服务调用，**在本机裸环境直接运行存在一定安全风险**（例如误操作影响宿主机文件、权限边界不清晰等）。

建议优先采用 Docker 运行，并遵循最小权限原则：

- 使用非 root 用户运行容器（或通过 `--user` 显式限制）
...
AuraID 的 **Skills 系统**是配置和扩展高级功能的首选方式。启动应用后，访问 `/skills` 页面即可导入和管理技能。
...
### 从 ClawHub 导入科学技能 / Scientific Skills from ClawHub

[ClawHub](https://github.com/OpenOwlab) 提供了一系列可复用的科学技能（Skills），包括实验自动化、文献挖掘、药物发现、蛋白结构分析等。你可以直接从 ClawHub 导入这些技能，也可以在此基础上编写自己的科学流 Skill。

**方式一：在 UI 中从 ClawHub 导入 / Import from ClawHub in UI**

1. 启动 AuraID 后，打开左侧导航中的 **Skills** 页面（`/skills`）
2. 点击右上角 **“从 ClawHub 导入 / Import from ClawHub”** 按钮
3. 在弹出的对话框中：
   - 搜索或粘贴你想要的 Skill 仓库地址（例如某个科学工作流的 `SKILL.md` 所在仓库）
   - 选择目标工作区，点击 **Import** 完成导入

导入完成后，这些 Skill 会出现在：

- **Skills 页面**：可查看、编辑元数据与配置
- **Agent 面板**：作为可调用工具，被智能体在对话中自动使用

**方式二：从任意 Git 仓库导入 / Import from Any Git Repo**

如果你在 ClawHub 之外维护了自己的科学流 Skill（例如含有 `SKILL.md` 的私有仓库），可以通过通用 Git 导入：

1. 打开 **Skills** 页面
2. 选择 **“从 Git 仓库导入 / Import from Git repository”**
3. 填写仓库地址（支持 HTTPS / SSH，视你的环境配置而定）
4. 选择要挂载的工作区，确认导入

**方式三：在本地创建你自己的科学流 Skill / Author Your Own Scientific Flow Skill**

1. 在任意仓库中新建 `SKILL.md`（可托管在 ClawHub、GitHub 或你的自建 Git 服务上）
2. 按照项目中的示例 `SKILL.md` 模板描述：
   - 能力说明（Capabilities）
   - 输入 / 输出参数（Inputs / Outputs）
   - 调用方式（如调用内部脚本、远程实验平台 API 等）
3. 将该仓库通过上面的导入方式接入 AuraID，对应 Skill 即可在 Agent 对话中被使用

配置完成后，你可以在 Agent 面板用自然语言触发这些技能，例如：

```text
> 从 ClawHub 导入一个用于药物靶点筛选的 Skill，基于当前工作区的候选分子做一次虚拟筛选，并输出 TOP 20 个结果
> 使用我自定义的实验自动化 Skill，生成一套基于当前论文的复现实验流程，并列出所需试剂与仪器清单
```
...
npm install -g pm2
npm run build
pm2 start npm --name "auraid" -- start
...
## 版权与来源 / Copyright & Origin

本仓库基于对 [SpectrAI-Initiative/InnoClaw](https://github.com/SpectrAI-Initiative/InnoClaw) 的二次开发，原始项目遵循 **Apache-2.0** 许可协议。
